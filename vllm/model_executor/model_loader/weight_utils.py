...
def gguf_quant_weights_iterator(
    gguf_files: str | list[str], gguf_to_hf_name_map: dict[str, str]
) -> Generator[tuple[str, torch.Tensor], None, None]:
    """
    Iterate over the quant weights in one or more GGUF files and convert
    ...
    # First pass: yield all weight types across all files
    ...
    # Second pass: yield all weight data across all files
    for reader in readers:
        for tensor in reader.tensors:
            if tensor.name in gguf_to_hf_name_map:
                weight = tensor.data
